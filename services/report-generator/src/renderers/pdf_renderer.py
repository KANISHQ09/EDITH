...
def render_pdf_isr(context: dict) -> bytes:
    ...
    h2_style = ParagraphStyle("H2", parent=styles["Heading2"], fontSize=13, spaceBefore=14, spaceAfter=6, textColor=ACCENT)
    ...
    if facts:
        ...
        for i, f in enumerate(facts, 1):
            ...
            elements.append(Paragraph(f"{i}. {f['content']} <font color='grey' size='8'>({conf}% confidence)</font>", body_style))
    ...
    if action_items:
        ...
        ai_data = [["#", "Description", "Owner", "Status"]] + [
            [str(i), a["content"][:60] + ("..." if len(a["content"]) > 60 else ""),
             a.get("ownerName") or "Unassigned", a["status"]]
    ...
    elements.append(Paragraph(
        f"Generated by VAIC — Voice AI Incident Commander · {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}",
        meta_style
    ))
